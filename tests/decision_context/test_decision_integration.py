...
from backend.results_store import ResultsStore
from backend.run_revision import review_outcome
...
def test_run_revision_guards_before_providers(monkeypatch):
    import backend.run_revision as rr
...
def test_decision_cli_checked_in_examples(tmp_path):
    ...
    examples = Path(__file__).resolve().parents[2] / "docs" / "examples" / "decision-context"
    schema = load_schema()
